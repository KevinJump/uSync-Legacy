...
namespace Jumoo.uSync.Core
{
	public class uSyncCoreConfig
	{
		...
		public uSyncCoreConfig()
		{
			try
			{
				var configFile = IOHelper.MapPath(Path.Combine(SystemDirectories.Config, "uSyncCore.config"));
			...
			{
				LogHelper.Warn<uSyncCoreConfig>("Unable to load the uSyncCore.config setting: {0}", () => ex);
			}
		...
		public void SaveSettings()
		{
			...
			{
				LogHelper.Warn<uSyncCoreConfig>("Unable to save the uSyncCore.config setting: {0}", () => ex);
			}
		}
	}
