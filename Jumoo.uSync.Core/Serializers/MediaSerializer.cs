...
namespace Jumoo.uSync.Core.Serializers
{
    public class MediaSerializer : ContentBaseSerializer<IMedia>
    {
        ...
        internal override SyncAttempt<IMedia> DeserializeCore(XElement node, int parentId, bool forceUpdate)
        {
            ...
            if (nodeGuid == null)
                return SyncAttempt<IMedia>.Fail(ChangeType.Import, "No guid, in xml");
            ...
            else
            {
                if (!forceUpdate)
                {
                    if (DateTime.Compare(update, item.UpdateDate.ToLocalTime()) < 0)
                        return SyncAttempt<IMedia>.Succeed(item, ChangeType.NoChange);
                }
            }
            ...
            return SyncAttempt<IMedia>.Succeed(item, ChangeType.Import);
